...
fn button_color(btn: StatusButton) -> ratatui::style::Color {
    match btn {
        StatusButton::Stage => ratatui::style::Color::Green,
        StatusButton::Unstage => ratatui::style::Color::Yellow,
        StatusButton::Discard => ratatui::style::Color::Red,
    }
}

fn button_char(btn: StatusButton) -> char {
    match btn {
        StatusButton::Stage => '+',
        StatusButton::Unstage | StatusButton::Discard => '-',
    }
}
...
fn append_button(spans: &mut Vec<Span<'static>>, btn: StatusButton, width: u16) -> u16 {
    ...
    spans.push(Span::styled(
        format!(" {}", button_char(btn)),
        Style::default().fg(button_color(btn)),
    ));
    btn_x as u16
}
